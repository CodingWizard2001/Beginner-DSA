...
int main() {
    ...
    while(testCase--){
        int lengthOfScore;
        cout<<"Enter the length of score : ";
        cin>>lengthOfScore;
        ...
        int scoreOfAlice = 0,scoreOfBob = 0;
        for(int i = 0;i < s.size();i++){
            if(i == 0 && s[i] == 'A') scoreOfAlice++;
            else if(s[i] == s[i - 1] && s[i] == 'A') scoreOfAlice++;
            else if(s[i] == s[i - 1] && s[i] == 'B') scoreOfBob++;
        }
        cout<<"Final score will be => "<<scoreOfAlice<<" : "<<scoreOfBob<<endl;
    }
}